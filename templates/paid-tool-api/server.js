// paid-tool-api — a paid summarizer backed by one versioned DeepSeek model choice inside Semesh's
// single public Model Service Unit. Discovery is anonymous; quote/invoke/observe/receipt are
// authenticated and payer-bound. Any missing or malformed canonical surface fails closed.
  ...
  if (options.authenticated) {
    if (!RUNTIME_KEY || !options.payer) {
      throw new ProtocolError("authentication_unavailable", "The authenticated payer rail is unavailable.", {
        beforeInvoke: !!options.beforeInvoke, status: 500,
      });
    }
...
async function discoverModelAction() {
  ...
  const meta = requireObject(search.meta, "invalid_search_contract", "Search omitted its Catalog metadata.", { beforeInvoke: true });
  const searchCatalog = catalogIdentityPin(meta.catalog_identity);
  if (!Array.isArray(items) || !exactIdentity(meta.catalog_token, 4096) || !searchCatalog) {
    throw new ProtocolError("invalid_search_contract", "Search omitted its bounded results, Catalog token, or identity.", { beforeInvoke: true });
  }
  ...
  const detailMeta = requireObject(detail.meta, "invalid_detail_contract", "Unit detail omitted Catalog metadata.", { beforeInvoke: true });
  ...
  const unit = requireObject(payload(detail), "invalid_detail_contract", "Unit detail is malformed.", { beforeInvoke: true });
  if (unit.id !== unitId || unit.kind !== "unit" || !Array.isArray(unit.actions)) {
  ...
  if (actions.length !== 1) throw new ProtocolError("model_action_ambiguous", "Detail must advertise one exact requested Action.", { beforeInvoke: true });
...
function assertPins(data, expected, phase, invocationId) {
...
function assertExecutionEvidence(data, expected, phase, invocationId) {
  assertPins(data, expected, phase, invocationId);
...
function receiptEvidence(json, expected) {
  const data = requireObject(payload(json), "invalid_receipt", "The canonical receipt response is malformed.", { invocationId: expected.invocationId });
  assertPins(data, expected, "receipt", expected.invocationId);
...
async function prepareCanonicalModelAction(input, payer, idempotencyKey) {
  ...
  const actionPath = `/v1/service-units/${encodeURIComponent(selected.unitId)}/actions/${encodeURIComponent(selected.actionId)}`;
  const quoteJSON = await fetchJSON("POST", `${actionPath}/quote`, {
    authenticated: true, payer, body: quoteBody, phase: "quote", beforeInvoke: true,
  });
  const quote = requireObject(payload(quoteJSON), "invalid_quote", "The authenticated quote response is malformed.", { beforeInvoke: true });
  assertPins(quote, selected, "quote");
...
function validatePreparedAction(prepared, idempotencyKey, payer) {
  const preparedKeys = ["action_path", "invoke_body", "deadline", "idempotency_key", "quote_evidence",
    "quoted_aev_atoms", "prepared_seal"].sort();
  ...
  if (prepared.action_path !== expectedPath) throw new ProtocolError("invalid_prepared_request", "The persisted Action path does not match its UnitActionRef.", { beforeInvoke: true });
  return { selected: {
...
async function observePreparedModelAction(prepared, payer, idempotencyKey, invocationId, expectedSettlementReference = null) {
  ...
  const actionPath = prepared.action_path;
  if (!exactIdentity(invocationId, 4096) || invocationId === idempotencyKey) {
    throw new ProtocolError("invalid_invocation_identity", "Invocation identity must be present and distinct from the replay key.", {
  ...
  const observeJSON = await fetchJSON("GET", `${actionPath}/invocations/${encodeURIComponent(invocationId)}`, {
    authenticated: true, payer, phase: "Invocation observation", invocationId,
  });
  const observation = requireObject(payload(observeJSON), "invalid_observation", "Invocation observation is malformed.", { invocationId });
  ...
  if (invocationIdentity(observation) !== invocationId || !exactIdentity(observation.settlement_reference, 4096) ||
    ...
    throw new ProtocolError("observation_identity_drift", "Observation did not preserve invocation and settlement identity.", { invocationId });
  }
...
async function invokePreparedModelAction(prepared, payer, idempotencyKey) {
  ...
  const invocation = requireObject(payload(invokeJSON), "invalid_invocation", "The invoke response is malformed.");
  ...
  if (!exactIdentity(invocation.settlement_reference, 4096)) throw new ProtocolError("invalid_invocation", "Invoke omitted its settlement reference.", { invocationId });
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/me" && req.method === "GET") return sendJSON(res, 200, { logged_in: !!payerToken(req) });

  if ((url.pathname === "/api/tool/quote" || url.pathname === "/api/tool/invoke" || url.pathname === "/api/tool/observe") && req.method === "POST") {
    const payer = payerToken(req);
    if (!payer) return sendJSON(res, 401, { error: "login_required", login: "/__semesh/login", message: "Sign in with Semesh first." });
    ...
    try {
      if (url.pathname === "/api/tool/quote") {
        const text = String(input.text || "").trim();
        if (!text) return sendJSON(res, 400, { error: "text_required" });
        if (text.length > 12000) return sendJSON(res, 413, { error: "text_too_long" });
        const style = String(input.style || "tldr").toLowerCase();
        if (style !== "tldr" && style !== "bullets") return sendJSON(res, 400, { error: "invalid_style" });
      ...
      const result = url.pathname === "/api/tool/observe"
        ? await observePreparedModelAction(input.prepared, payer, idempotencyKey, input.invocation_id)
        : await invokePreparedModelAction(input.prepared, payer, idempotencyKey);
      return sendJSON(res, 200, { ok: true, style: input.style, ...result });
    } catch (error) {
      const known = error instanceof ProtocolError;
      const beforeInvoke = url.pathname !== "/api/tool/observe" && (url.pathname === "/api/tool/quote" || (known && error.beforeInvoke));
      const status = known && error.status === 402 ? 402 : (beforeInvoke ? 503 : 502);
      return sendJSON(res, status, {
        error: known ? error.code : "tool_failed",
        message: known ? error.message : "The canonical outcome is unknown; replay only the exact same request with the same Idempotency-Key.",
...
if (require.main === module) server.listen(PORT, () => console.log(`paid-tool-api listening on :${PORT} (base ${BASE})`));
